...
export default async function SpecialistDetailsPage({
  ...
  params: Promise<{ specialistId: string }>;
    ...
    const { data: doctor } = await getSpecailistDetails(specialistId);
  if (!doctor) {
    return notFound();
  }
        ...
        <CardContent>
          <p className="text-muted-foreground">
            {`Dr. ${doctor.name
              .split(" ")
              .slice(1)
              .join(" ")} is a highly respected ${doctor.specialty} based at ${
              doctor.hospital
            } in ${
              doctor.location
            }. With over 10 years of experience, they are dedicated to providing the highest standard of care.`}
          </p>
        </CardContent>
      </Card>

...
export default function PublicFacilityDetailsPage() {
  ...
  const providerId = params.facilityId as string;
  ...
  useEffect(() => {
    ...
    async function fetchFacility() {
      const { data, error } = await getFacilities({ id: providerId });
      if (error) {
        ...
        <CardContent className="space-y-4">
          {facility.services.length > 0 ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No specific services listed for this provider yet. Please call for
              more information.
            </div>
          )}
        </CardContent>

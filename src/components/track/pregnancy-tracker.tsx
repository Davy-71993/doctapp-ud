...
export function PregnancyTracker({
  isPregnantInitially = false,
}: PregnancyTrackerProps) {
  const [isPregnant, setIsPregnant] = useState(isPregnantInitially);

  const handlePrenancySttus = (status: boolean) => {
    localStorage.setItem(
      "healthDataPregnancy",
      JSON.stringify({ status: status ? "Pregnant" : "Not pregnant" })
    );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Track pregnancy status for tailored advice.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex items-center space-x-4 rounded-md border p-4">
          ...
          <Switch
            checked={isPregnant}
            onCheckedChange={handlePrenancySttus}
            id="pregnancy-status"
        ...
        {isPregnant && (
          <div className="mt-4 text-center p-4 bg-secondary rounded-lg">
            ...
            <p className="text-sm text-muted-foreground">
              Remember to schedule regular check-ups with your gynecologist.
            </p>
          </div>
        )}
      </CardContent>
